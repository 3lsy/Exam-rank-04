...
int main(int ac, char **av, char **ev)
{
	...
	while (av[i] && av[i + 1])
	{
		...
		//count arg
		ac = 0;
		while (av[ac] && strcmp(av[ac], "|") && strcmp(av[ac], ";")) //skip program name or "|" or ";"
			ac++;
		i = ac;

		//check if cd is the first
		if (strcmp(av[0], "cd") == 0)
		{
			if (ac != 2)
		...
		//check if cmd is not proceeded by pipe and execute it
		...
		//check if cmd is proceeded by pipe
